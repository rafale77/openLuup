...
--        local star = implemented_actions[name] and '*' or ''
  ...
  local j, err = json.encode ({serviceList = S}, {empty_table_as_array=true, pretty=true})
...
local function iprequests ()
  local ips = json.encode({ip_requests = iprequests_table ()}, {empty_table_as_array=true, pretty=true})
...
local function sdata(...)
  ...
  return json.encode (sdata, {empty_table_as_array=true, pretty=true}) or 'error in sdata', "application/json"
end
--
-- STATUS
--
...
--        (info[1].id or _) = nil
  ...
  return json.encode (status, {empty_table_as_array=true, pretty=true}) or 'error in status', "application/json"
end
...
local function user_data (_,p)
  ...
  then
    ...
    result = json.encode (user_data2, {empty_table_as_array=true, pretty=true})
  end
...
local function static ()
  ...
  local result = json.encode (sd, {empty_table_as_array=true, pretty=true})
  return result, mime_type
end
...
local function action (_,p,f)
  ...
  if error ~= 0 then
    ...
    if f == "json" then
      result = json.encode (result, {empty_table_as_array=true, pretty=true})
      mime_type = "application/json"
...
local function update_plugin (_,p)
  ...
  if meta then
    ...
    local arg = {metadata = json.encode (meta, {empty_table_as_array=true, pretty=true})}
